...
public class ZoneOneNodeMapController : NodeMapController
{
        ...
        for(int i = 1; i < 10; i++)
        {
            ...
            else if (data.d_zone_one_ld.level_status_arr[i] == 4)
            {
                ...
                else if (i <= 4)
                {
                    ...
                    if (i != 4)
                    {
                        data.d_zone_one_ld.level_status_arr[i + 1] = 1;
                    }
                    data.d_zone_one_ld.level_status_arr[(i + 5) % 10] = 1;
                }
                else
                {
                    //buttons[i - 5].image.color = Color.cyan;
                ...
                data.d_zone_one_ld.level_status_arr[i] = 2;
            }
        }
    }
    ...
    public static List<TurnHandler.CharType> GetEnemyBotArray(string node)
    {
        switch (node)
        {
            ...
            case "Node Seven":
                return new List<TurnHandler.CharType>() { TurnHandler.CharType.DPS, TurnHandler.CharType.Tank, TurnHandler.CharType.Healer };
            case "Node Eight":
                return new List<TurnHandler.CharType>() { TurnHandler.CharType.Healer, TurnHandler.CharType.Tank, TurnHandler.CharType.Healer };
            case "Node Nine":
