...
public class ZoneThreeNodeMapController : NodeMapController
{
    protected override void Start()
    {
        ...
        for(int i = 1; i < 10; i++)
        {
            ...
            if (data.d_zone_three_ld.level_status_arr[i] == -1)
            {
                //buttons[i - 1].image.color = Color.cyan;
                buttons[i - 1].image.sprite = broken_cpus[(i - 1) % 3];

            }
            else if (data.d_zone_three_ld.level_status_arr[i] == 1)
            {
                //buttons[i - 1].image.color = Color.yellow;
                buttons[i - 1].interactable = true;
            }
            else if (data.d_zone_three_ld.level_status_arr[i] == 2)
            {
            ...
            else if (data.d_zone_three_ld.level_status_arr[i] == 3)
            {
                //buttons[i - 1].image.color = Color.yellow;
                buttons[i - 1].interactable = true;
                ...
                else if (i <= 4)
                {
                    //buttons[i + 3].image.color = Color.cyan;
                    buttons[i + 3].image.sprite = broken_cpus[(i + 3) % 3];
                    data.d_zone_three_ld.level_status_arr[(i + 4) % 10] = -1;
                }
                else
                {
                    //buttons[i - 5].image.color = Color.cyan;
                    ...
                    data.d_zone_three_ld.level_status_arr[i - 4] = -1;
                }
            }
            else if (data.d_zone_three_ld.level_status_arr[i] == 4)
            {
                ...
                else if (i <= 4)
                {
                    ...
                    if (i != 4)
                    {
                        data.d_zone_three_ld.level_status_arr[i + 1] = 1;
                    }
                    data.d_zone_three_ld.level_status_arr[(i + 5) % 10] = 1;
                }
                else
                {
                    //buttons[i - 5].image.color = Color.cyan;
    ...
    {
        data.d_zone_index = 3;
        data.d_zone_three_ld.level_index = node;
    ...
    public static List<TurnHandler.CharType> GetEnemyBotArray(string node)
    {
        switch (node)
        {
            ...
            case "Node Three":
                return new List<TurnHandler.CharType>() { TurnHandler.CharType.Tank, TurnHandler.CharType.Tank, TurnHandler.CharType.Tank };
            case "Node Four":
            ...
            case "Node Seven":
                return new List<TurnHandler.CharType>() { TurnHandler.CharType.DPS, TurnHandler.CharType.DPS, TurnHandler.CharType.DPS };
            case "Node Eight":
