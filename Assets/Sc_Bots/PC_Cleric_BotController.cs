...
public class PC_Cleric_BotController : BaseBotController
{
    public override void Setup(bool isPlayerTeam, TurnHandler instance, int index)
    {
        ...
        actionOneName = "Jab";
        actionTwoName = "Holy Word";
        actionThreeName = "Resurrect";
            ...
            for (int i = 0; i < bots.Length; i++)
            {
                if (i != self)
                {
                    if (IsValidTarget(3, bots[i].controller))
                    {
                        ...
                        {
                            // +4 Value for Dead Target
                            Debug.Log("AI: " + self.ToString() + " detected dead AI: " + i.ToString());
    ...
    public override bool IsValidTarget(int action, BaseBotController target)
    {
        switch (action)
        {
            case (1):
                ...
                Debug.Log(botName + "thinks action " + action.ToString() + " on target " + target.ToString() + " is invalid");
                return false;
            case (2):
                if (target.IsAlive())
                {
    ...
    public override bool ActionThreeCallback(BaseBotController target)
    {
        if (SpendEnergy(actionThreeCost) && !target.IsAlive())
        {
            ...
            {
                Debug.Log("Attemped to Rez Alive Target");
                return false;
            }
    ...
    public override IEnumerator ActionThreeMain()
    {
        ...
        curTarget.Revive(actionThreeDamage);
