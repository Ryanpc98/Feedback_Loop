...
public class PC_Cowboy_BotController : BaseBotController
{
    public override void Setup(bool isPlayerTeam, TurnHandler instance, int index)
    {
        ...
        actionTwoDamage = 20;
        actionTwoCost = 30;
        actionThreeDamage = 120;
        actionThreeCost = 120;

        actionOneName = "Six Shooter";
        actionTwoName = "Giddy Up";
        actionThreeName = "High Noon";

        actionOneType = ActionType.Attack;
        actionTwoType = ActionType.Buff;
    ...
    public override void ActionTwoValue(TurnHandler.BotInfo[] bots, int self)
    {
        ...
        if (CanDoActionTwo())
        {
            // +1 Value for being able to do it
            Debug.Log("AI: " + self.ToString() + " detected action two is possible");
            decisions.Add(new DecisionInfo { target = self, action = 2 });
            if (actionThreeCost > 1)
            {
                // +1 Value for having room to move action three cost
                Debug.Log("AI: " + self.ToString() + " detected action three is above 1 cost is possible");
                decisions.Add(new DecisionInfo { target = self, action = 2 });
                if (actionThreeCost >= 6)
                {
                    // +1 Value for having action 3 cost over 6
                    Debug.Log("AI: " + self.ToString() + " detected action 3 cost over 6");
                    decisions.Add(new DecisionInfo { target = self, action = 2 });
                }
            }
        }
    ...
    public override IEnumerator ActionTwoMain()
    {
        ...
        ChangeActionCost(3, -1 * (int)actionTwoCost);
