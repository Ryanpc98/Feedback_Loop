...
public class PC_Ninja_BotController : BaseBotController
{
    public override void Setup(bool isPlayerTeam, TurnHandler instance, int index)
    {
        ...
        actionOneName = "Sneaky Stab";
        actionTwoName = "Twin Strike";
        actionThreeName = "Deceive";
    ...
    public override void ActionThreeValue(TurnHandler.BotInfo[] bots, int self)
    {
        ...
        if (CanDoActionThree())
        {
            // +1 Value for Having the Energy to do the action
            Debug.Log("AI: " + self.ToString() + " has the energy for Deceive");
            decisions.Add(new DecisionInfo { target = self, action = 3 });
    ...
    public override IEnumerator ActionTwoMain()
    {
        ...
        Debug.Log("Action 2 Waiting For First");
        yield return new WaitUntil(() => state == State.Attacking);

        Debug.Log("Executing Action 2 First");
        StartCoroutine(curTarget.FlashColor(flashColor, time));
        ...
        Debug.Log("Action 2 Waiting For Idle");
        yield return new WaitUntil(() => state == State.Idle);

        state = State.Sliding;

        Debug.Log("Action 2 Waiting For Second");
        yield return new WaitUntil(() => state == State.Attacking);

        Debug.Log("Executing Action 2 Second");
        StartCoroutine(curTarget.FlashColor(flashColor, time));
        handler.PlaySFX(actionTwoSFX);
        curTarget.ApplyDamage(actionTwoDamage);
        if (!IsTargetable())
    ...
    public override bool ActionThreeCallback(BaseBotController target)
    {
        ...
        {
            curTarget = this;
            if (curTarget.GetID() != GetID())
            {
                Debug.Log("Attemped to Deceive Someone Else");
                return false;
            }
